...
class WITWindow(Adw.ApplicationWindow):
    def __init__(self, **kw):
        ...
        self._app_reinstall_btns: dict[str, Gtk.Button] = {}  # pkg -> reinstall button
    ...
    def _build_header(self):
        ...
        left.append(wm); left.append(tl); left.append(ver); left.append(dev)
        hdr.append(left)
    ...
    def _make_app_card(self, app):
        ...
        reinstall_btn.set_valign(Gtk.Align.CENTER); reinstall_btn.set_margin_end(10)
        reinstall_btn.set_tooltip_text("Reinstall")
        reinstall_btn.set_visible(False)
        reinstall_btn.connect("clicked", lambda _, a=app, b=btn: self._on_install(a, b, reinstall=True))
        self._app_reinstall_btns[app["pkg"]] = reinstall_btn

        card.append(icon_lbl); card.append(info); card.append(btn); card.append(reinstall_btn)
        outer.append(card)
        return outer
        ...
        def worker():
            ...
            try:
                ...
                if self.sudo_ok and os.path.exists(self._askpass_path):
                    ...
                    GLib.idle_add(self._term_write,
                        f"[debug] at-run-time askpass exists={os.path.exists(self._askpass_path)} "
                        f"sudo_ok={self.sudo_ok}\n", "dim")
                else:
                    GLib.idle_add(self._term_write,
                        f"[debug] askpass NOT injected — sudo_ok={self.sudo_ok} "
                        f"askpass_exists={os.path.exists(self._askpass_path)}\n", "err")
    ...
    def _apply_sudo_result(self, ok):
        ...
        if ok:
            ...
            # --- TEMP DEBUG: verify the file actually landed on disk ---
            _exists = os.path.exists(self._askpass_path)
            _mode = oct(os.stat(self._askpass_path).st_mode) if _exists else "N/A"
            self._term_write(
                f"[debug] askpass_path={self._askpass_path} exists={_exists} mode={_mode}\n",
                "dim")
            # --- END TEMP DEBUG ---
